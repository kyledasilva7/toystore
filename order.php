<?php   										// Opening PHP tag
	...
	function get_info(PDO $pdo, string $id) {
		...
		$sql = "SELECT *
			...
			WHERE ordernum= :id AND
			customer.custnum= orders.custnum;";	// :id is a placeholder for value provided later 
		                               // It's a parameterized query that helps prevent SQL injection attacks and ensures safer interaction with the database.


		// Execute the SQL query using the pdo function and fetch the result
		$info1 = pdo($pdo, $sql, ['id' => $id])->fetch();		// Associative array where 'id' is the key and $id is the value. Used to bind the value of $id to the placeholder :id in  SQL query.
